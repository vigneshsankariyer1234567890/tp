...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/teletubbies/logic/command` directory.
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `InputParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
Create a `RemarkCommandParser` class in the `teletubbies.logic.parser` package. The class must extend the `Parser` interface.
...
:information_source: Don’t forget to update `InputParser` to use our new `RemarkCommandParser`!
...
Create a new `Remark` in `teletubbies.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`teletubbies.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
